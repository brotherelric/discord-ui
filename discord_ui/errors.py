...
class InvalidLength(commands.BadArgument):
    ...
    def __init__(self, my_name, _min=None, _max=None, *args: object) -> None:
        ...
        super().__init__(err)
class OutOfValidRange(commands.BadArgument):
...
class WrongType(commands.BadArgument):
...
class InvalidEvent(commands.BadArgument):
...
class MissingListenedComponentParameters(commands.BadArgument):
...
class CouldNotParse(commands.BadArgument):
